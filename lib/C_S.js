...
var supportList = {
    ...
    'rpc_server_heartbeat': function() {
        if(CACHE.battle.battleType === 0) {
            ...
            if(CACHE.autoPlay.cooperateAdvertType && CACHE._times > 0) {
                ...
                }else  if(CACHE.startPVPGame==true){
                    if(CACHE.autoPlay.pvpAdvertType ) {
                        echo('开始竞技！',)
                        res = "";
                        if (CACHE.dragonBallInfo.dragonBallDeckIndex !==1){
                            res = 'server.rpc_server_dragon_ball_switch_deck(1);'
                        }
                        //res += 'client.rpc_client_hang_up_verify(1,[1]);'
                        return res+'server.rpc_server_start_game(1)'; // 竞技模式
                    }
                }
    ...
    'rpc_server_hang_up_verify':function(list){
