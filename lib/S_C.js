...
var ballconfig = require('../utils/gameData.cfg.dragon.ball')
/**
 * -- 服务器
    server  -- 主服务器
    battle_server   -- 对战服务器
...
var supportList = {
    ...
    'rpc_client_login_return': function (ret) {
        ...
        CACHE.battle.MonsterCurrentCount = 20
        CACHE.battle.round = 0; // 战斗第几回合
        CACHE.battle.currentRound = 0;
        CACHE.battle.MonsterMergeCount=555;
        return result;
    },
    /**
     * 开始游戏
    ...
    // 公告
	'rpc_client_single_mission':function(){
    ...
    'rpc_client_single_achieve':function(){
        //{ k: 96, v: 7 } { k: 11, v: 26 } { k: 13, v: 7 } { k: 5, v: 18 } { k: 1, v: 1312 } { k: 6, v: 10 } { k: 17, v: 8 } { k: 96, v: 7 } { k: 11, v: 26 } { k: 13, v: 7 } { k: 5, v: 18 } { k: 1, v: 1312 } { k: 6, v: 10 } { k: 17, v: 8 }
        // echo('openbox')
        // if( CACHE.openCount >0){
        //     CACHE.openCount -=1;
        //     setTimeout(function(){
        //         CACHE.postData.push('server.rpc_server_battle_cooperate_open_box(1);')
        //     },3*1000);
    ...
    'rpc_client_market_daily_special_info': function(data) {
        var result = "",
            autoPayList = [1, 2, 3,138,139,140,141,142,143,144,145,146,147,148,149,150,151,152,153,154,155,167];
        //var result = "",
        //    autoPayList = [1, 2, 3,138,139,140,141,142,143,144,145,146,147,148,149,150,151,152,153,154,155,167];
        if(data && data.length) {
            for (var i=0; i < data.length ;i++) {
                ...
                if([166,168].includes(data[i].k) && CACHE.userInfo["diamond"]>488){
                    CACHE.userInfo["diamond"] = CACHE.userInfo["diamond"] - 488;
                    result += "server.rpc_server_market_daily_special_buy(" + data[i].k + ");";
                    echo('[花费钻石购买] ID: ', data[i].k);
                }else{
                    echo('钻石不足停止购买')
                }
            }
    ...
    'rpc_client_dragon_ball_deck': function(data, using) {
        ...
        CACHE.dragonBallInfo.dragonBallDeckIndex = using;
    },
    // 获得奖励 - 处理奖励领取金币和宝箱
    'rpc_client_common_reward': function(from, rewardShow, rewardAdd, boxKey) {
        ...
        for(var i in rewardAdd){
            if(ballconfig[rewardAdd[i]["k"]-2]){
                if(CACHE.boxReward[ballconfig[rewardAdd[i]["k"]-2]["name"] ] ){
                    CACHE.boxReward[ballconfig[rewardAdd[i]["k"]-2]["name"] ] +=rewardAdd[i]["v"];
                }else{
                    CACHE.boxReward[ballconfig[rewardAdd[i]["k"]-2]["name"] ] =rewardAdd[i]["v"];
                }
            }else if(rewardAdd[i]["v"]==1){
                var name = rewardAdd[i]["k"]

                if(rewardAdd[i]["k"] ==98){
                    name = "连击球球"
                }

                if(rewardAdd[i]["k"] ==106){
                    name = "奔流球球"
                }
                if(rewardAdd[i]["k"] ==112){
                    name = "天使球球"
                }
                if(rewardAdd[i]["k"] ==131){
                    name = "蝴蝶球球"
                }
                if(rewardAdd[i]["k"] ==241){
                    name = "迷雾球球"
                }
                if(rewardAdd[i]["k"] ==245){
                    name = "赤龙球球"
                }
                if(rewardAdd[i]["k"] ==246){
                    name = "光棱球球"
                }
                if(rewardAdd[i]["k"] ==247){
                    name = "神龙球球"
                }
                if(rewardAdd[i]["k"] ==248){
                    name = "毒爆球球"
                }
                if(rewardAdd[i]["k"] ==249){
                    name = "魔典球球"
                }
                if(CACHE.boxReward[name] ){
                    CACHE.boxReward[name ] +=rewardAdd[i]["v"];
                }else{
                    CACHE.boxReward[name ] =rewardAdd[i]["v"];
                }
            }
        ...
        if( CACHE.openCount >0){
            echo('openbox')
                ...
                CACHE.openCount -=1;
                return 'server.rpc_server_battle_cooperate_open_box(1);'; // 开箱子模式
 
            
            
            
        }else{
            echo(CACHE.boxReward)
        }
    ...
    'rpc_client_legion_competition_map_info':function(){

    },
    
    'rpc_client_king_match_hall_of_fame':function(){

    },
    'rpc_client_achieve_list':function(){

    },
    'rpc_client_legion_competition_conflict_info':function(){},

    'rpc_client_chatroom_enter_rv': function(id,succ,subchannel,num){

        echo('进入聊天室状态：',succ,'聊天室编号：',subchannel,'当前人数:',num)
    },
    'rpc_client_hang_up': function(isMulti, questionStr, options){
        CACHE.isVerify=true;
         //local BattlePvpResultPopLayer = require("ui.battle_scene.battle_pvp_result_pop_layer");BattlePvpResultPopLayer:onOKClick();
         echo(questionStr,options);
         CACHE.verifyOptions = options;
         CACHE.verifyQuestion = questionStr;
         echo('--------')
         if(CACHE.verifyCache){
            CACHE.verifyCache[questionStr]=options;
         }
         else{
            CACHE.verifyCache = {}
            CACHE.verifyCache[questionStr]=options;
         ...
         //return 'client.rpc_client_hang_up_verify(1,['+toString( options[1])+']);local HangUpData = require("module.pto_data.hang_up_data");HangUpData:rpc_client_hang_up_verify(1,[1]);';
    
    },
    'rpc_client_hang_up_verify':function(result,list){
        if (result==1){
            echo("verify success!")
            CACHE.isVerify=false;
        }
    },
};
// 服务器 -> 客户端
