...
var supportList = {
    ...
    'rpc_client_market_daily_special_info': function(data) {
        var result = "",
            autoPayList = [1, 2, 3,138,139,140,141,142,143,144,145,146,147,148,149,150,151,152,153,154,155,167,166,168];
        //var result = "",
    ...
    'rpc_client_common_reward': function(from, rewardShow, rewardAdd, boxKey) {
        ...
        for(var i in rewardAdd){
            if(ballconfig[rewardAdd[i]["k"]-2]){
                ...
                if(CACHE.boxReward[rewardAdd[i]["k"]] ){
                    CACHE.boxReward[rewardAdd[i]["k"] ] +=rewardAdd[i]["v"];
                }else{
                    CACHE.boxReward[rewardAdd[i]["k"] ] =rewardAdd[i]["v"];
                }
            }
