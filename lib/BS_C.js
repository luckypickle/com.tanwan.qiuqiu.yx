        ...
        if(playInfo.cfg) {
            ...
            var get_not_merge = function(){
                valueList = Object.values(playInfo.ballsGrade).filter( (item) => {
                    // 非满级 且 不在不升级的纯辅助球球中
                    return gameData.BattleConst.notMerge.includes(item.dbType);
                });
                return valueList.length;
            }
            if(CACHE.battle.battleType!=1&&(CACHE.battle.currentRound<200 ||get_not_merge()==0)){
                return;
            }
