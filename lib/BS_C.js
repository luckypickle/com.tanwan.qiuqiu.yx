...
<<<<<<< HEAD
        if(CACHE.stopMerge){
=======
        if(CACHE.stopMerge == true){
>>>>>>> 77219400b054bce441bf09a9b060487112e29897
            return;
        }
        ...
        if(CACHE.battle.killBallMergeTime > 0 && CACHE.battle.runTimeLeft - CACHE.battle.killBallMergeTime < 3000) {
            if( CACHE.battle.is_kill_boss && CACHE.battle.battleType !=2){
                echo("暗杀大师球球判定");
        ...
        if(playInfo.cfg) {
            ...
            var getAllDuBaoBall_list = function(){
                return  Object.values(ballList).filter( (ballItem) => {
                    // 棋盘包含万能球球
                    return ballItem.ballType == 68 ;
                });
            }
            ...
            if (CACHE.battle.battleType == 1){
                ...
                allAttachBall =getAllAttachBall_list();
                ...
                dubao_ball_stars = 0;
                allduBao = getAllDuBaoBall_list();
                for(i=0;i<allduBao.length;i++){
                    dubao_ball_stars += allduBao[i].star;
                }
                if((allAngerList.length==3||allAngerList.length==5||allAngerList.length==7 )&&dubao_ball_stars>35){
                    echo("检测毒爆超多输出模式，退出买球")
                        return;
                }
            ...
            // 取棋盘是否存在万能球球
            var getAllPowenfulBallList = function(cost) {
                return Object.values(ballList).filter( (ballItem) => {
                    if (cost >100){
                        return gameData.BattleConst.allPowerful.includes(ballItem.ballType);
                    }else{
            ...
            allPowenfulBallList = getAllPowenfulBallList(playInfo.cfg.cost);
            // 判断存在万能球，优先合并
            if(allPowenfulBallList.length > 0 &&CACHE.battle.otherStart) {
            ...
            if(CACHE.battle.battleType==1 &&CACHE.battle.otherStart){
                ...
                var mergeKillBall = function(_keyList,is_forbid) {
                    var evalStr = "";
                    for (i=0; i < _keyList.length ; i++) { // 循环所有球球，寻找可合并的
                        key = _keyList[i];
                        var ballItem = CACHE.getKillBallMergeId(key,is_forbid);
                        // 存在可以合并的球球
                        if(ballItem) {
                            evalStr += 'battle_server.rpc_server_fight_ball_merge(' + key + ',' + ballItem.ballId + ');';
                ...
                keyList = CACHE.getBallKeysSort();
                result = mergeKillBall(keyList,allGuang.length);
                if(result) {
                    return result;
                }
            }
